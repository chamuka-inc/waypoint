...
import type { AppState, Opportunity, Profile, Fit, FeedbackKind, Application, ApplicationDraft, RuntimeStatus, UpdateInfo, ResearchRun, WorkspaceBootstrap, WorkspaceSummary, CreateWorkspaceInput } from './types.js';
import { priorityScore, salary, constraints } from './ranking.js';
import { blankProfile } from './demo.js';
import './style.css';

declare global { interface Window { waypoint?: { call: (method: string, ...args: unknown[]) => Promise<unknown>; checkForUpdates: () => Promise<unknown> } } }
...
const draftEdits: Record<string, { application: string; resume: string }> = {};
let draftArtifact: 'application' | 'resume' = 'application';
...
async function checkForUpdates(manual = false) {
  ...
  updateChecking = true; updateError = false;
  if (page === 'settings') render();
  try {
    updateInfo = window.waypoint
      ? await window.waypoint.checkForUpdates() as UpdateInfo
      : await call<UpdateInfo>('checkForUpdates');
    if (manual) toast(updateInfo.available ? `Waypoint ${updateInfo.latestVersion} is available.` : 'No newer Waypoint release is available.');
  } catch {
    updateError = true;
    if (manual) toast('Could not check for updates. Check your connection and try again.');
  } finally {
    updateChecking = false;
    if (page === 'settings' || updateInfo?.available) render();
  }
}
...
function navigate(next: string) {
  if (draftDirty && !confirm('Leave without saving your draft changes?')) return;
  ...
  page = next; selected = null; draftDirty = false; modal = null; workspaceMenu = false; query = ''; selectedRoleIds.clear(); render(); window.scrollTo(0, 0);
}
function openDialog() { focusBeforeDialog = document.activeElement as HTMLElement; }
function closeDialog() { if (draftDirty && !confirm('Close without saving your draft changes?')) return; selected = null; draftDirty = false; modal = null; render(); focusBeforeDialog?.focus(); }
...
function updateSettings() {
  const label = updateChecking ? 'Checking…' : updateInfo?.available ? 'Update available' : updateError ? 'Check failed' : updateInfo ? 'Up to date' : 'Not checked';
  const description = updateError ? 'The release check could not reach GitHub. Your work is unaffected.' : updateInfo?.available
    ? `Waypoint ${esc(updateInfo.latestVersion)} is available. You’re using ${esc(updateInfo.currentVersion)}.`
    : updateInfo ? `You’re using Waypoint ${esc(updateInfo.currentVersion)}. No newer published release is available.`
    : 'Waypoint checks GitHub for new releases when the app starts and while it remains open.';
  return `<section class="form-panel"><div class="settings-title"><span class="integration-icon">${icon('package-check')}</span><div><h2>App updates</h2><p>New Waypoint releases</p></div><span class="status-pill ${updateInfo?.available ? 'completed' : ''}">${label}</span></div><p>${description}</p><p class="small-note">Updates are downloaded and installed by you from the GitHub release page.</p><div class="button-row"><button class="button secondary" data-action="check-updates" ${updateChecking ? 'disabled' : ''}>${icon('refresh-cw')}Check for updates</button>${updateInfo?.available ? `<a class="button primary" href="${esc(updateInfo.releaseUrl)}" target="_blank" rel="noopener noreferrer">View release ${icon('arrow-up-right')}</a>` : ''}</div></section>`;
}
function aboutWaypoint() {
  return `<section class="form-panel"><div class="settings-title"><span class="integration-icon">${icon('route')}</span><div><h2>About Waypoint</h2><p>The project behind your workspace</p></div></div><p>Explore the Waypoint website for features, documentation, and downloads. Share ideas or report problems through the project’s GitHub Issues.</p><div class="button-row"><a class="button secondary" href="https://chamuka-inc.github.io/waypoint/" target="_blank" rel="noopener noreferrer">${icon('globe-2')}Visit website ${icon('arrow-up-right')}</a><a class="button secondary" href="https://github.com/chamuka-inc/waypoint/issues" target="_blank" rel="noopener noreferrer">${icon('message-circle')}GitHub Issues ${icon('arrow-up-right')}</a></div><p class="small-note">Issues are public. Leave personal profile and CV details out of posts.</p></section>`;
}
...
function draftDrawer(role: Opportunity) {
  const record = state.drafts.find(item => item.roleId === role.id);
  const edits = draftEdits[role.id] || (record ? { application: record.edited.application, resume: record.edited.resume } : { application: '', resume: '' });
  const current = edits[draftArtifact];
  const reviewed = draftArtifact === 'application' ? record?.applicationReviewed : record?.resumeReviewed;
  const stale = record && (record.profileRevision !== state.profileRevision || record.sourceUrl !== (role.sources[0]?.url || '') || (record.sourceCheckedAt || '') !== (role.sources[0]?.checkedAt || '') || record.company !== role.company || record.title !== role.title);
  return `<div class="overlay" data-backdrop="true"><section class="detail-drawer draft-drawer" role="dialog" aria-modal="true" aria-labelledby="draft-title"><div class="drawer-toolbar"><span>${icon('file-pen-line')} APPLICATION DRAFTS</span><button class="icon-button" data-action="close" aria-label="Close drafts">${icon('x')}</button></div><div class="drawer-header"><small>${esc(role.company)} · ${esc(role.title)}</small><h2 id="draft-title">Make this application yours.</h2><p class="body-copy">Codex can prepare a starting point. Check every claim before using it.</p><p class="small-note">Profile revision ${state.profileRevision} · ${role.sources[0] ? `Vacancy checked ${date(role.sources[0].checkedAt)} · <a href="${esc(role.sources[0].url)}" target="_blank" rel="noopener noreferrer">Open source</a>` : 'Vacancy details supplied by you'}. Contact details missing from your profile can be added in the editor.</p>${record ? `<span class="status-pill ${record.status === 'ready' ? 'completed' : ''}" role="status">${esc(record.status === 'running' ? 'Codex is drafting…' : record.status === 'ready' ? 'Drafts saved' : record.error || record.status)}</span> <small>Updated ${date(record.updatedAt)}</small>${stale ? '<p class="small-note">Your profile or opportunity changed since generation. Review or regenerate these drafts.</p>' : ''}` : ''}</div><div class="drawer-body draft-body">${record?.status === 'running' ? `<p>Preparing your application and tailored CV. You can leave this view while Codex works.</p><button class="button secondary" data-action="cancel-draft">Cancel generation</button>` : ''}${record?.pending ? `<div class="callout">A new version is ready. Your edits are still saved. Review the new version before replacing them. <button class="button secondary" data-action="accept-draft">Use new version</button></div><details><summary>Preview generated version</summary><h4>Application</h4><pre>${esc(record.pending.application)}</pre><h4>CV</h4><pre>${esc(record.pending.resume)}</pre></details>` : ''}${record && (record.edited.application || record.edited.resume) ? `<div class="detail-tabs" role="tablist"><button role="tab" aria-selected="${draftArtifact === 'application'}" class="${draftArtifact === 'application' ? 'active' : ''}" data-draft-artifact="application">Application draft</button><button role="tab" aria-selected="${draftArtifact === 'resume'}" class="${draftArtifact === 'resume' ? 'active' : ''}" data-draft-artifact="resume">Tailored CV</button></div><div class="draft-layout"><div><label class="form-field"><span>${draftArtifact === 'application' ? 'Application statement' : 'Tailored CV'}</span><textarea id="draft-text" rows="20" data-draft-text="${draftArtifact}">${esc(current)}</textarea></label><div class="draft-controls"><button class="button secondary" data-action="save-draft">Save edits</button><button class="button secondary" data-action="review-draft">${reviewed ? 'Reviewed' : 'Mark reviewed'}</button><label class="draft-export-choice"><span>Format</span><select id="draft-format" aria-label="Export format"><option value="docx">DOCX</option><option value="pdf">PDF</option><option value="txt">Plain text</option></select></label><button class="button primary" data-action="export-draft" ${reviewed && !draftDirty ? '' : 'disabled'}>${icon('download')}Export</button></div><small>${draftDirty ? 'Unsaved changes' : reviewed ? 'Reviewed by you' : 'Needs your review before export'}</small></div><aside class="draft-evidence"><h3>Evidence & questions</h3><p class="small-note">Check that each example describes your own work accurately.</p>${record.edited.evidence.map(item => `<div class="evidence-card supported"><strong>${esc(item.claim)}</strong><p>${esc(item.source)}</p><small>${esc(item.requirement)}</small></div>`).join('')}${record.edited.questions.length ? `<h4>Questions to resolve</h4>${list(record.edited.questions)}` : ''}</aside></div>` : ''}<section class="draft-setup"><h3>${record?.edited.application ? 'Regenerate from current inputs' : 'Create an application draft and tailored CV'}</h3><p>Uses your saved profile, this opportunity and its vacancy source. Your information goes to your configured Codex provider. Waypoint will not submit anything.</p><form id="draft-form"><label class="form-field"><span>What should Codex emphasise? (optional)</span><textarea name="focus" rows="3" maxlength="4000" placeholder="A specific, truthful achievement or aspect of the role"></textarea></label><label class="form-field"><span>Vacancy description (optional)</span><textarea name="description" rows="4" maxlength="30000" placeholder="Paste the job description if the source is incomplete"></textarea></label><button class="button primary" type="submit" ${state.demo || record?.status === 'running' || runtime?.codex === false ? 'disabled' : ''}>${icon('sparkles')}${record?.edited.application ? 'Regenerate drafts' : 'Create drafts with Codex'}</button></form>${state.demo ? '<p class="small-note">Create your own profile and research a real vacancy to use this feature.</p>' : runtime?.codex === false ? '<p class="small-note">Codex was not detected. Check Settings → Codex.</p>' : ''}</section></div></section></div>`;
}
function detailDrawer() {
  const role = state.roles.find(r => r.id === selected);
  if (!role) { selected = null; return ''; }
  if (detailTab === 'draft') return draftDrawer(role);
...
function applyBootstrap(bootstrap: WorkspaceBootstrap) {
  ...
  selected = null; selectedRoleIds.clear(); draft = null; dirty = false; draftDirty = false; for (const id of Object.keys(draftEdits)) delete draftEdits[id]; for (const id of Object.keys(notesDraft)) delete notesDraft[id]; modal = null; workspaceMenu = false; query = ''; fitFilter = 'all'; modeFilter = 'all'; salaryFilter = false; hideConflicts = false;
}
async function switchWorkspace(id: string) {
  ...
  if (activeRun() || state.drafts.some(draft => draft.status === 'running')) { toast('Finish or cancel Codex work before switching workspaces.'); return; }
  busy = true;
...
async function removeSelectedRoles() {
  ...
  const draftCount = state.drafts.filter(draft => ids.includes(draft.roleId)).length;
  if (!ids.length || !confirm(`Remove ${ids.length} selected ${ids.length === 1 ? 'opportunity' : 'opportunities'}? Shortlist entries, application notes, and feedback for them will also be removed.${draftCount ? ` This will also delete ${draftCount} saved application ${draftCount === 1 ? 'draft' : 'drafts'}.` : ''}`)) return;
  busy = true;
...
async function saveDraftEditor() {
  ...
  const record = state.drafts.find(item => item.roleId === selected);
  if (!record) return false;
  const edits = draftEdits[selected] || { application: record.edited.application, resume: record.edited.resume };
  try {
    state = await call('saveDraft', { id: selected, revision: record.revision, ...edits });
    draftDirty = false; render(); toast('Draft edits saved.'); return true;
  } catch (e) { toast((e as Error).message); return false; }
}

function closeSelectControls(except?: HTMLElement) {
  document.querySelectorAll<HTMLElement>('[data-select-control].open').forEach(control => {
    if (control === except) return;
    control.classList.remove('open');
    control.querySelector<HTMLElement>('.select-menu')!.hidden = true;
    control.querySelector<HTMLElement>('.select-trigger')!.setAttribute('aria-expanded', 'false');
  });
}
function openSelectControl(control: HTMLElement, focusEdge: 'selected' | 'first' | 'last' = 'selected') {
  closeSelectControls(control);
  control.classList.add('open');
  control.querySelector<HTMLElement>('.select-menu')!.hidden = false;
  control.querySelector<HTMLElement>('.select-trigger')!.setAttribute('aria-expanded', 'true');
  const options = [...control.querySelectorAll<HTMLButtonElement>('[role="option"]')];
  const option = focusEdge === 'first' ? options[0] : focusEdge === 'last' ? options.at(-1) : options.find(item => item.getAttribute('aria-selected') === 'true');
  option?.focus();
}
...
root.addEventListener('click', async event => {
  ...
  const selectOption = target.closest<HTMLButtonElement>('[data-select-value]');
  if (selectOption) {
    const control = selectOption.closest<HTMLElement>('[data-select-control]')!;
    const proxy = control.querySelector<HTMLSelectElement>('.select-proxy')!;
  ...
  const selectTrigger = target.closest<HTMLButtonElement>('[data-select-trigger]');
  if (selectTrigger) {
    const control = selectTrigger.closest<HTMLElement>('[data-select-control]')!;
    if (control.classList.contains('open')) closeSelectControls(); else openSelectControl(control);
    return;
  }
  closeSelectControls();
  ...
  if (button.dataset.openDraft) { openDialog(); selected = button.dataset.openDraft; detailTab = 'draft'; draftArtifact = 'application'; render(); return; }
  ...
  if (button.dataset.draftArtifact === 'application' || button.dataset.draftArtifact === 'resume') { draftArtifact = button.dataset.draftArtifact; render(); return; }
  ...
  switch (button.dataset.action) {
    ...
    case 'open-draft': detailTab = 'draft'; draftArtifact = 'application'; render(); break;
    case 'cancel-draft': await mutate('cancelDraft'); break;
    case 'save-draft': await saveDraftEditor(); break;
    case 'review-draft': {
      if (draftDirty && !(await saveDraftEditor())) break;
      const record = state.drafts.find(item => item.roleId === selected);
      if (record) await mutate('reviewDraft', [record.roleId, record.revision, draftArtifact], 'Marked reviewed.');
      break;
    }
    case 'accept-draft': {
      const record = state.drafts.find(item => item.roleId === selected);
      if (draftDirty && !confirm('Use the new generated version and discard your unsaved edits?')) break;
      if (record) { draftDirty = false; delete draftEdits[record.roleId]; await mutate('acceptDraft', [record.roleId, record.revision], 'New draft version is ready for review.'); }
      break;
    }
    case 'export-draft': {
      const record = state.drafts.find(item => item.roleId === selected);
      if (!record || draftDirty || !(draftArtifact === 'application' ? record.applicationReviewed : record.resumeReviewed)) break;
      const name = `waypoint-${record.company}-${record.title}-${draftArtifact}-${new Date().toISOString().slice(0, 10)}`.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      try {
        const format = (document.querySelector<HTMLSelectElement>('#draft-format')?.value || 'docx') as 'docx' | 'pdf' | 'txt';
        const { exportDraftText } = await import('./draft-export.js');
        await exportDraftText(record.edited[draftArtifact], name, format);
    ...
    case 'check-updates': await checkForUpdates(true); break;
    case 'dismiss-update': dismissedUpdate = updateInfo?.latestVersion || ''; render(); break;
...
root.addEventListener('input', event => {
  ...
  if (input.id === 'draft-text' && selected) {
    const record = state.drafts.find(item => item.roleId === selected);
    if (record) { draftEdits[selected] ||= { application: record.edited.application, resume: record.edited.resume }; draftEdits[selected][draftArtifact] = input.value; draftDirty = true; }
  }
  if (input.id === 'search') {
...
root.addEventListener('change', async event => {
  const input = event.target as HTMLInputElement;
  if (input.closest('#profile-form')) dirty = true;
...
root.addEventListener('submit', async event => {
  ...
  if (form.id === 'draft-form' && selected) { await mutate('startDraft', [selected, String(fd.get('focus')), String(fd.get('description'))], 'Codex is preparing your drafts.'); }
});
document.addEventListener('keydown', event => {
  const keyTarget = event.target as HTMLElement;
  const selectControlElement = keyTarget.closest<HTMLElement>('[data-select-control]');
  if (selectControlElement && event.key === 'Escape' && selectControlElement.classList.contains('open')) {
    event.preventDefault(); closeSelectControls(); selectControlElement.querySelector<HTMLButtonElement>('.select-trigger')?.focus(); return;
  }
  if (keyTarget.matches('.select-trigger') && ['Enter', ' ', 'ArrowDown', 'ArrowUp'].includes(event.key)) {
    event.preventDefault(); openSelectControl(selectControlElement!, event.key === 'ArrowUp' ? 'last' : 'selected'); return;
  }
  if (keyTarget.matches('[role="option"]') && selectControlElement) {
    const options = [...selectControlElement.querySelectorAll<HTMLButtonElement>('[role="option"]')];
    const index = options.indexOf(keyTarget as HTMLButtonElement);
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') { event.preventDefault(); options[(index + (event.key === 'ArrowDown' ? 1 : -1) + options.length) % options.length]?.focus(); return; }
...
window.addEventListener('beforeunload', event => { if (dirty || draftDirty) event.preventDefault(); });
async function boot() {
  try {
    ...
    setInterval(async () => {
      if (!activeRun() && !state.drafts.some(draft => draft.status === 'running')) return;
      const workspaceId = currentWorkspace.id; const epoch = workspaceEpoch;
      try { const fresh = await call<AppState>('getState'); if (workspaceId !== currentWorkspace.id || epoch !== workspaceEpoch) return; const researchCompleted = Boolean(activeRun()) && !fresh.runs.some(r => r.status === 'running'); const draftCompleted = state.drafts.some(d => d.status === 'running' && fresh.drafts.find(next => next.roleId === d.roleId)?.status !== 'running'); state = fresh; if ((!selected && !modal && page !== 'profile') || (selected && detailTab === 'draft' && draftCompleted)) render(); if (researchCompleted) toast(fresh.runs[0].status === 'completed' ? 'Your research is ready to explore.' : fresh.runs[0].error || 'Research stopped.'); if (draftCompleted) toast('Draft generation finished. Review the content and evidence.'); } catch { /* transient fetch failure leaves last-known state visible */ }
    }, 1800);
