...
import type { AppState, Opportunity, Profile, Fit, FeedbackKind, Application, RuntimeStatus } from './types.js';
...
function navigate(next: string) {
  ...
  page = next; selected = null; modal = null; query = ''; render(); window.scrollTo(0, 0);
}
...
function savedPage() {
  const roles = currentRoles();
  return `${pageHeading('A LITTLE CLOSER TO YOUR NEXT MOVE', 'Your shortlist', 'Keep the possibilities you want to come back to.', `<button class="button secondary" data-action="export-shortlist">${icon('download')}Export shortlist</button>`)}${toolbar()}<div class="saved-grid" id="role-list">${roles.length ? roles.map(roleCard).join('') : empty('Make room for possibility', 'Save an opportunity to keep its evidence, questions, and next steps close.', 'Explore opportunities', 'discover')}</div>`;
}
...
root.addEventListener('submit', async event => {
  ...
  if (form.id === 'settings-form') {
    try { state = await call('settings', { jevEnabled: fd.has('enabled'), jevModel: String(fd.get('model')).trim() }, String(fd.get('key')).trim() || undefined); runtime = await call<RuntimeStatus>('status'); render(); toast('AI settings saved.'); } catch (e) { toast((e as Error).message); }
  }
